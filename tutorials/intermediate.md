...
The trade-off works like a seesaw. Simple models — linear regression, shallow decision trees, Naive Bayes — impose strong assumptions on the data. Those assumptions are rarely perfectly true, so the model makes a systematic error (bias). But because the model is rigid, it does not react wildly to different training samples, so its predictions are stable (low variance).

Complex models — deep decision trees, k=1 KNN — make almost no assumptions. They can represent nearly any function, so bias is low. But their flexibility means they latch onto noise in the training data, and a slightly different training set produces dramatically different predictions (high variance).
...
Choosing between all these algorithms can feel overwhelming, but a few practical heuristics carry you a long way. Always start with a linear model. Linear regression for continuous targets, logistic regression for classification — they are fast, interpretable, and set a baseline that every other model should beat before you pay the cost of complexity. If the linear model falls short, tree ensembles are usually the next move. Random forest is a robust default: it handles mixed feature types, requires minimal tuning, and rarely catastrophically overfits. If you need more accuracy and are willing to tune, gradient boosting (or XGBoost) is typically the strongest classical model on structured tabular data.

The nature of your data matters too. If you are working with text, Naive Bayes or logistic regression with TF-IDF features is a natural starting point — both are fast and interpretable, and logistic regression often edges ahead with more data. If your features live in a very high-dimensional space, apply PCA before training: it removes redundant dimensions that slow training and can confuse distance-based methods like KNN and SVM. If you have no labels at all and want to explore the structure of your data, K-Means is the simplest first step — run it with several values of k and inspect the clusters before deciding whether to add more complexity.

SVM is worth reaching for when you have a small-to-medium dataset with clear margins between classes, and when you are willing to invest time in choosing the right kernel and tuning regularisation. KNN is appealing for its simplicity but scales poorly to large datasets and suffers badly in high dimensions — use it as a sanity check rather than a production model unless the data is small and well-scaled.

## When to Use It

Classical machine learning is the right tool for a wide range of problems, and knowing when it is enough is just as important as knowing the algorithms themselves. If your data is tabular — rows of observations with named columns — tree ensembles and linear models are hard to beat, and they are far easier to deploy, explain, and debug than neural networks. If your dataset is small (hundreds to low thousands of rows), classical models are almost always preferable: deep learning needs large amounts of data to justify its complexity, and on small datasets it will overfit where a regularised linear model or a random forest would generalise.

Classical ML also wins on interpretability. Logistic regression coefficients, decision tree rules, and feature importances from random forests are all things you can show to a stakeholder or a regulator. A neural network's predictions are harder to explain, and in high-stakes domains — medicine, finance, law — that explainability gap is a real cost.

That said, there are clear signals that it is time to move on to deep learning. If your input is raw images, audio, or text (rather than hand-crafted features), neural networks learn representations that no amount of feature engineering can match. If your dataset is very large (hundreds of thousands of rows or more), deep learning can continue to improve where classical models plateau. And if the problem involves sequences — time series, natural language, speech — recurrent and transformer architectures handle temporal dependencies in ways that tree models fundamentally cannot. When you find yourself hitting those walls, the Deep Learning track is the natural next step.
...
You have now covered the full classical machine learning toolkit: linear models that draw interpretable boundaries, tree ensembles that combine many weak learners into something powerful, probabilistic classifiers, kernel methods, and unsupervised techniques for clustering and dimensionality reduction. Along the way you have built intuition for the bias-variance trade-off — the central idea that ties every algorithm together — and learned to ask not just "does this model fit?" but "will it generalise?" That shift in thinking is what separates a practitioner from someone who merely runs code. The tools you have now are genuinely useful in the real world, and many production systems run on exactly the models you have studied. When you are ready to go further — when images, language, or scale push classical ML to its limits — the Deep Learning track picks up right where this one leaves off.
